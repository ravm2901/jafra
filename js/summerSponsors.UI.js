	...
	$.UI_Sponsor = function(options) {
		UI_sponsor = {
			...
			registerUIActions: function() {
				...
				UI_sponsor.options.btnReturn.click(function(e){
					e.preventDefault();
			...
			_getInfoFields: function(){				
				$.ajax({
					url:  CONSTANTS.rootURL + CONSTANTS.pgmSponsor,
					async: true,
					...
					beforeSend: function(obj){
						...
						timeLoad = setTimeout(function(){
							UI_sponsor.options.divError.html('<i class="fa myspin fa-refresh"></i> ' + translate.commons.timeToLoad);
							UI_sponsor.options.divError.css('display', 'block');
							
							tooTimeLoad = setTimeout(function(){
								obj.parent().find('td:nth-child(4)').html('<i class="fa myspin fa-refresh"></i> ' + translate.commons.tooTimeToLoad);
								UI_sponsor.options.divError.css('display', 'block');
					...
					error: function(obj, what, otherObj){	
						...
						UI_sponsor.options.divError.html('Error loading information. Please check your internet connection and try again.');
						UI_sponsor.options.divError.css('display', 'block');
					...
					if(response.ListConsultant.length > 0){
						...
						UI_sponsor._buildSponsor(response.ListConsultant);
						
					}
					else{			
						//UI_sponsor.options.divError.html('Consultant not found.');
						//UI_sponsor.options.divError.css('display', 'block');
						UI_sponsor.options.tableSponsors.find('tbody').append('<tr><td colspan="9">Data not found</td></tr>');
					}
			...
			_buildSponsor: function(data){
				...
				try{
					...
					for(var i = 0; i < data.length; i++){
						...
						UI_sponsor.options.tableSponsors.find('tbody').append(tr);
					}
			...
			_setElements_UI: function(){
				...
				try{
					...
					$.getScript( scriptLanguage ).done(function( script, textStatus ) {
						...
						// translate page
						UI_sponsor.translateUI();
						UI_sponsor._getInfoFields();
						UI_sponsor.registerUIActions();
		...
		return {
            init             : UI_sponsor.init,
			options          : UI_sponsor.options,
            registerUIActions: UI_sponsor.registerUIActions,
			translateUI      : UI_sponsor.translateUI
